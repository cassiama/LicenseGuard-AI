...
- `LANGCHAIN_TRACING_V2`: a boolean (either `true` or `false`). It should look like this: `true`
...
- `POST /generate/report`: Accepts a `requirements.txt` file upload, a project name & the user's authentication token, analyzes each license associated with the dependencies in the `requirements.txt` file, and ***streams*** the risk report based on the analysis.
  - Sample Request:
    - Form Inputs:
      - a `requirements.txt` (`multipart/form-data`; should be `text/plain` MIME type),
      - a name for your project
      - the JWT for the current user
      > NOTE: This will change to be less terrible later on, this is a temp solution. It is not expected for someone to interact with this image in the terminal yet; it's meant to interact with the frontend.
  ...
  - Sample Response:
    ...
    - Success:
        ...
        * **Conflict**: [GPL] vs [MIT]
            * **Source**: `PyQt5`
            * **Risk**: GPL requires the entire project to be open-sourced.

        ### 🪜 Actionable Next Steps
        **Recommended Swaps:**
        * **Problem**: `PyQt5` (GPL)
            * **Solution**: Switch to `PySide6` (LGPL).
            * **Migration Note**: LGPL allows you to link dynamically without open-sourcing your own proprietary code.
    ...
    - Failure:
        ...
        ### ⚠️ License Conflicts
        No obvious license conflicts detected here.
