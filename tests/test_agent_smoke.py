...
This test mocks all external dependencies (LLM and MCP server) to ensure the agent graph can be initialized and executed without real API keys.
"""
...
def mock_dependencies(monkeypatch):
    ...
    mock_llm_chain.ainvoke = AsyncMock(return_value=AIMessage(content="Mocked Summary"))
...
async def test_agent_smoke_test(mock_env, mock_dependencies):
    ...
    assert final_report.content == "Mocked Summary", (
